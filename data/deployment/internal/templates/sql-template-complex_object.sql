...
CREATE TABLE IF NOT EXISTS `{$tmp_tablename}` (
  ...
  `VERS` INTEGER NOT NULL DEFAULT 1 COMMENT 'Version de l objet',
  `REV` INTEGER NOT NULL DEFAULT 0 COMMENT 'Revision de l objet',
  ...
  CONSTRAINT `FK_{$tmp_OBJCODE}UACCT_UUSER`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `UQ_{$tmp_OBJCODE}_BIDVERSREV` UNIQUE(`BID`,`VERS`,`REV`))
ENGINE = InnoDB,
STATS_AUTO_RECALC=1
COMMENT = 'Table des .';
...
BEGIN
  ...
  DECLARE l_sBID VARCHAR(200);
  SELECT CAST( TID AS CHAR CHARACTER SET utf8) INTO l_sTIDOBD FROM A000_OBD WHERE OBI_DB_TABLENAME = '{$tmp_tablename}';
  SET l_sBID = CONCAT(NEW.BID,'_',LPAD(CONVERT(NEW.VERS,CHAR),2,'0'),'.',LPAD(CONVERT(NEW.REV,CHAR),2,'0'));

  INSERT INTO A100_OBI(TID,OBD_TID,BID) VALUES (NEW.TID,l_sTIDOBD,l_sBID);
END$$
